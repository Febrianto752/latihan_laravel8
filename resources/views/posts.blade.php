...
<div class="card mb-5 text-center">
    ...
    @else
    <img src="/images/banner-web-1.jpg" class="card-img-top" alt="{{ $posts[0]->category->name }}"
        style="object-fit: cover; height: 400px;">

    @endif
    <div class="card-body">
        <h3 class="card-title"><a href="/posts/{{ $posts[0]->slug }}"
                class="text-decoration-none text-secondary">{{ $posts[0]->title }}</a></h3>
        <p class="card-text mb-0">Author : <a
                href="/posts?author={{ $posts[0]->author->username }}">{{ $posts[0]->author->name }}</a>, Kategori : <a
...
<div class="row">
    ...
    <div class="col-md-4">
        <div class="position-absolute px-3 py-3 rounded" style="background-color: rgba(0, 0, 0, .6); z-index: 1;"><a
        ...
        <div class="card mb-4">
            ...
            <div class="card-body">
                <h5 class="card-title"><a href="/posts/{{ $post->slug }}"
                        class="text-decoration-none text-secondary">{{ $post->title }}</a></h5>
                <p class="card-text">Author : <a
                        href="/posts?author={{ $post->author->username }}">{{ $post->author->name }}</a></p>
